#!/bin/zsh
#
# twingate-enforce-remediate.sh — BNDRY MDM Twingate LaunchAgent install
...
if [[ "$ZOMBIE_COUNT" -gt 0 && -n "$CONSOLE_USER" && "$CONSOLE_USER" != "root" ]]; then
    sudo -u "$CONSOLE_USER" osascript -e \
      'display notification "Twingate accumulated stuck network adapters. Please restart your Mac when convenient to fully clear them." with title "BNDRY IT" sound name "Sosumi"' \
      2>/dev/null
fi
