...
```mermaid
graph TB
    subgraph Device["🖥️ macOS Developer Device"]
        Script["sbom-audit-spdx<br/>(Monthly MDM Run)"]
        Nix["sbomnix<br/>Scans Nix Profile"]
        Syft["Syft<br/>Scans Homebrew + 20 Ecosystems"]
        PySDPX["pyspdxtools<br/>Local Validation"]
    ...
    subgraph AWS["☁️ Security AWS Account"]
        Lambda["Lambda Function<br/>device-sbom-audit"]
        S3["S3 Bucket<br/>device-sbom-audit.security.example.com"]
        
        subgraph S3Prefixes["S3 Prefixes"]
            Uploads["uploads/<br/>(temporary)"]
            SBOMs["sboms/<br/>(validated)"]
        end
        
        Lifecycle["Lifecycle Policies<br/>• uploads/ → 1d delete<br/>• sboms/ → 90d Glacier<br/>• sboms/ → 1y delete"]
    ...
    Script -->|"1️⃣ POST /presign<br/>Auth: SBOM_AUTH_TOKEN<br/>Body: {device_id, checksum}"| Lambda
    Lambda -->|"2️⃣ Presigned PUT URL<br/>HMAC sig in metadata<br/>30min expiry"| Script
    Script -->|"3️⃣ PUT to S3<br/>metadata: device-id, checksum, sig"| Uploads
    Uploads -->|"4️⃣ S3 Event<br/>ObjectCreated"| Lambda
    Lambda -->|"5️⃣ Validate SPDX<br/>• Schema check<br/>• HMAC verify<br/>• Checksum verify"| Uploads
    Lambda -->|"6️⃣ Copy if valid<br/>SSE-AES256"| SBOMs
    Lambda -->|"7️⃣ Delete source"| Uploads
    
    classDef deviceStyle fill:#1a1a2e,stroke:#16213e,color:#eee,stroke-width:2px
    classDef awsStyle fill:#232f3e,stroke:#ff9900,color:#eee,stroke-width:2px
    classDef storageStyle fill:#0f3460,stroke:#16a085,color:#eee,stroke-width:2px
    classDef lambdaStyle fill:#1a472a,stroke:#27ae60,color:#eee,stroke-width:2px
    
    class Device deviceStyle
    class AWS awsStyle
    class S3,S3Prefixes,Uploads,SBOMs storageStyle
...
```mermaid
sequenceDiagram
    participant Device as 🖥️ macOS Device<br/>(AS-laptop-123)
    participant Lambda as ⚡ Lambda Function<br/>(device-sbom-audit)
    participant S3 as 📦 S3 Bucket<br/>(device-sbom-audit.security.example.com)
    
    Note over Device: Monthly scheduled run<br/>Random delay: 0-3600s
    
    Device->>Device: 1. Scan Nix packages (sbomnix)
    Device->>Device: 2. Scan system packages (Syft)
    Device->>Device: 3. Merge & deduplicate
    Device->>Device: 4. Validate with pyspdxtools
    Device->>Device: 5. Calculate SHA-256 checksum
    
    rect rgb(26, 26, 46)
        Note over Device,Lambda: Authentication & Presign
        Device->>Lambda: POST /presign<br/>Auth: SBOM_AUTH_TOKEN<br/>{device_id, checksum}
        Lambda->>Lambda: Validate token (constant-time)<br/>Sanitize device_id<br/>Validate checksum format
        Lambda->>Lambda: Generate HMAC signature<br/>sig = HMAC-SHA256(secret, checksum)
        Lambda-->>Device: {upload_url, correlation_id}<br/>Metadata includes sig
    end
    
    rect rgb(15, 52, 96)
        Note over Device,S3: Direct Upload (Bypasses 6MB Lambda Limit)
        Device->>S3: PUT to presigned URL<br/>metadata: device-id, checksum, sig<br/>Body: SPDX JSON (up to 75MB)
        S3-->>Device: 200 OK
    end
    
    rect rgb(26, 71, 42)
        Note over S3,Lambda: Validation & Archival
        S3->>Lambda: S3 Event: ObjectCreated<br/>Key: uploads/AS-laptop-123/...
        Lambda->>S3: GetObject (with 30s timeout)
        alt Object exists
            S3-->>Lambda: SBOM body + metadata
            Lambda->>Lambda: Re-sanitize device_id<br/>Verify HMAC sig (constant-time)<br/>Verify checksum (constant-time)<br/>Validate SPDX 2.3 schema<br/>Check package limits
            alt Validation passes
                Lambda->>S3: CopyObject to sboms/<br/>(SSE-AES256, new metadata)
                Lambda->>S3: DeleteObject from uploads/
                Note over Lambda: ✅ SBOM archived
            else Validation fails
                Lambda->>S3: DeleteObject from uploads/
                Note over Lambda: ❌ Invalid SBOM deleted
            end
        else NoSuchKey (already processed)
            Note over Lambda: ✓ Idempotent - skip
        end
    end
    
    Note over S3: Lifecycle: uploads/ → 1d delete<br/>sboms/ → 90d Glacier → 1y delete
```
...
**Optional:** subscribe to alerts via `terraform apply -var "alert_email=secops@example.com"`. No subscription is created by default to keep `terraform plan` diff-free.
...
- **S3 buckets must be empty.** Versioned delete markers count — clear with `aws s3 rm s3://<bucket> --recursive` followed by deleting noncurrent versions, or use the AWS console.
- **Secrets Manager has a 7-day recovery window** by default. The secret names cannot be reused for re-creates within that window. Use `aws secretsmanager delete-secret --force-delete-without-recovery` if you need to recreate immediately.
- **The KMS customer-managed key has a 30-day deletion window.** Schedule deletion separately if you want it gone faster.
    ...
    "Statement": [
      {
        ...
        "Resource": "arn:aws:s3:::device-sbom-audit.security.example.com/uploads/*"
      },
      {
        "Sid": "WriteOnceArchive",
        "Effect": "Allow",
        "Action": ["s3:GetObject", "s3:PutObject"],
        "Resource": "arn:aws:s3:::device-sbom-audit.security.example.com/sboms/*"
      }
    ]
  }'
```
...
aws lambda create-function \
  ...
  --environment Variables="{
    \"SBOM_BUCKET\":\"device-sbom-audit.security.example.com\",
...
aws lambda add-permission \
  ...
  --source-arn arn:aws:s3:::device-sbom-audit.security.example.com
...
aws s3api get-bucket-policy --bucket device-sbom-audit.security.example.com
...
aws s3 cp /tmp/test-upload.json \
  s3://device-sbom-audit.security.example.com/uploads/test/fake.json 2>&1
...
aws s3 ls s3://device-sbom-audit.security.example.com/sboms/ --recursive
...
**Weekly Checks:**
- Review S3 storage size: `aws s3 ls s3://device-sbom-audit.security.example.com/sboms/ --recursive --summarize`
...
aws s3 ls s3://device-sbom-audit.security.example.com/uploads/ --recursive
...
aws s3api delete-bucket-policy --bucket device-sbom-audit.security.example.com
```
...
This implementation is provided as-is for internal use. SPDX is a registered trademark of the Linux Foundation.
