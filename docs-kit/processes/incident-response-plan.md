---
Artefact type: Plan
Owner role: IR Coordinator (or equivalent; whoever activates the plan), sponsored by the most senior technical leader
...
- Support a smooth transition between infrastructure or service providers if necessary.
...
| Step | Description |
| --- | --- |
| Contact the IR Coordinator | The person who activates this plan and notifies the response team. |
| Assessment | The IR Coordinator assesses the situation and consults senior leadership. |
| Decision | The IR Coordinator decides whether to activate, based on severity and impact. |
...
| Role | Function |
| --- | --- |
| IR Coordinator | Activates the plan; overall response coordination. |
...
Keep named contacts for each role (phone, email, and a fallback channel) in a document reviewed at least annually and kept separate from this plan, so contact details can be updated without revising the plan itself.
...
If the organisation serves customers regulated by the **Australian Prudential Regulation Authority (APRA)**, a material incident may trigger a customer's own **CPS 234** notification obligation (the customer must notify APRA as soon as possible and, in any case, no later than 72 hours after becoming aware); even where the organisation itself isn't APRA-regulated, having a process to support a customer's compliance obligation is worth building in.
