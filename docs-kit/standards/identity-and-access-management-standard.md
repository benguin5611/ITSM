...
Reference numbering starts each control area at the next hundred (e.g. Identity and Access Management IAM-100–199, Controlled Access Based on Need-to-Know IAM-200–299), so a reader can tell which control area a reference belongs to at a glance.
...
| Ref | Statement |
| --- | --- |
| IAM-100 | Implement a layered access control strategy combining Mandatory Access Control (MAC), Discretionary Access Control (DAC), and Role-Based Access Control (RBAC) to secure system components and data resources. |
...
| IAM-109 | Disable dormant accounts after a period of at least twelve (12) months. |
...
| IAM-111 | Before deleting accounts, confirm 'disabled' status has been authorised for 'deletion'. |
...
| IAM-118 | Ensure all temporary access has an expiration date that is monitored and enforced. |
...
- **Review cadences (IAM-300/301)** — 12 months for general access, 3 months for privileged access on critical systems — are a common baseline, not a universal floor. Some frameworks or regulators mandate shorter cycles for specific data classes or sectors.
...
**Frameworks referenced** (by family — check the current edition of whichever applies to you): NIST's digital identity guidelines (SP 800-63 series), the OWASP Password Storage Cheat Sheet, and ISO/IEC 27001 Annex A's access-control objectives.
