...
jobs-to-be-done optionally draws on: write-like-a-human (final voice pass)

standalone (no cross-references): knowledge-matrix · help-centre-article · business-case · custom-json-schema · playwright-demo-recorder
```

A referenced skill is a **preference**: if it isn't installed, the orchestrator falls back to a generic pass and names the skill as the tool it would have reached for. The lens skills are equally usable on their own.

## Using these skills

The exact loading mechanism depends on your runtime:

- **Claude Code**: drop the folder into `~/.claude/skills/` or your project's skills directory. The `/<skill-name>` invocation name comes from the directory name; the frontmatter `name:` field only sets the display label in skill listings.
...
- Supporting material lives under `references/`, `templates/`, or `scripts/` (custom-json-schema keeps its two pluggable companions at the skill root).
- No company-specific names, internal tools, or proprietary content.

## Licence

MIT: see [../LICENSE](../LICENSE). One exception: [`write-like-a-human/references/ai-writing-signs.md`](write-like-a-human/references/ai-writing-signs.md) adapts Wikipedia content and is licensed CC BY-SA 4.0 instead (see the notice at the top of that file).
