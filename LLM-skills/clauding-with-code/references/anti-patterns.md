...
## A23. Narrated-forensics comments drowning the code
**Symptom:** a diff's comment lines outweigh its code lines — one real instance had roughly half the added lines in a `.github/workflows` change as comments, with a single-line `@main` pin buried under ten lines of prose walking through the investigation that led to it.
**Why it bites:** a comment that narrates the debugging journey (what was tried, what failed, why the author eventually landed here) reads as load-bearing but isn't — it turns a one-line mechanism into a paragraph the reviewer has to wade through, and it rots the moment the surrounding code moves.
**Instead:** a comment earns its place only by stating a *load-bearing why* the code itself can't express — a hidden constraint, an invariant, a workaround for a specific bug, a non-obvious consequence — in one line or a short few, not a narrative. The forensic detail (what you tried, what you ruled out, how you found the fix) belongs in the PR description or commit body, read once, not in the source, read every time. If deleting a comment wouldn't leave a future reader confused, delete it. First-class requirement, not a style nit: enforce it on every reviewed diff.
